...
import { UploadForecast } from "@/api/revenu";
...
export default {
  ...
  data() {
    return {
      loading: false,
      downloadUrl: process.env.BASE_API + "/space/template?filename=RevenuPlan",
      tableData: undefined,
      AllTypeOptions: [
        {
          Id: 1,
  ...
  methods: {
    ...
    UploadData() {
      ...
      this.tableData.forEach(function(item) {
        var targetObs = wcs.filter(p => p.Name == item.WC);
      ...
      UploadForecast(this.tableData).then(response => {
        if (response.code == 20000) {
          ...
          Message({
            message: 'revenu upload success',
            type: "success",
            duration: 5 * 1000
          });
        } else {
          this.$message({
             message: 'revenu upload error',
            type: "error",
            duration: 5 * 1000
          });
        }
