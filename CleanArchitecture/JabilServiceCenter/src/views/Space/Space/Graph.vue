    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      <el-main style="padding:0; margin:0;">
        <el-container>
          ...
          <el-main>
            <div>
              <h2>{{titleSummary}} Monthly Summary Report/Group By WCs</h2>
              <el-table
                :key="tableKey"
...
import { getRevenuSummary } from "@/api/revenu";
...
export default {
  ...
  methods: {
    drawLine() {
      ...
      var type = { type: "bar" };
      var objSeries = [];
      this.chartMap.forEach(o => objSeries.push(type));
      //var objLists = [];
      //var objLists =this.objList.slice(0);
      //var obj = this.objList.splice(this.objList.length-3,3);
      this.objList.forEach(element => {
        element.WorkcellId = this.GetWCName(element.WorkcellId);
      });

      var obj = this.objList;
      if (this.$route.query.type) {
         if(this.$route.query.type == 'space')
         {
           obj = this.objList.splice(0, this.objList.length - 3);
           this.titleSummary = 'Space';
         }
         else
         {
           this.titleSummary = 'Revenu/Space';
         }
       }

      var demn = this.chartMap.unshift("WorkcellId");
      // var obj = this.list.splice(this.list.length - 3, 3);
      var option = {
        legend: {},
        tooltip: {},
        dataset: {
          dimensions: this.chartMap,
          source: obj
        },
        xAxis: { type: "category" },
        yAxis: {},
    ...
    getListsByQuery(query) {
      ...
      if (gtype === "space") {
        getSummaryForecastListsByWcs(query).then(response => {
          ...
          this.chartMap = JSON.parse(JSON.stringify(this.tableHeader));
          this.objList = JSON.parse(JSON.stringify(this.list)); // 完美 this.list.slice(0);
        ...
        getRevenuSummary(query).then(response => {
          debugger;
          this.list = response.data;
          this.tableHeader = response.header;
          this.total = response.total;
